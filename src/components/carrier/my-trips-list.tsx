...
export function MyTripsList({ trips, isLoading, onEdit }: MyTripsListProps) {
  ...
  return (
    <>
          ...
          return (
            ...
            >
              ...
              {trip.status === 'In-Transit' ? (
                ...
                <div className="flex gap-2 mt-2">
                  <Button variant="outline" size="sm" className="gap-2 flex-1" onClick={() => setManifestTrip(trip)} disabled={isThisTripProcessing}>
                    <Users className="h-4 w-4" />
                    <span>{t('manifest')}</span>
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => onEdit(trip)} className="text-blue-600 h-9 w-9" disabled={isThisTripProcessing}><Edit className="h-4 w-4" /></Button>
      ...
      <TripManifestDialog
        tripId={manifestTrip?.id ?? null}
        trip={manifestTrip}
        open={!!manifestTrip}
        onOpenChange={(open) => !open && setManifestTrip(null)}
      />
    </>
  );
}