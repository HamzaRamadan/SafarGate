...
export function AskAiDialog() {
  ...
  const handleSend = async () => {
    ...
    setMessages(prev => [...prev, { id: `user-${Date.now()}`, role: 'user', content: userQuestion }]);
    setInput('');
    setIsLoading(true);
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: `assistant-${Date.now()}`, role: 'assistant', content: answerText }]);
    } catch (error) {
      setMessages(prev => [...prev, { id: `error-${Date.now()}`, role: 'assistant', content: "عذراً، حدث خطأ في الاتصال." }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
            ...
            <span className="sr-only">اسأل المساعد الذكي</span>
        </Button>
      </DialogTrigger>
            ...
            <div className="space-y-4">
                {messages.length === 0 && (
                    <div className="text-center text-sm text-muted-foreground mt-10 space-y-2">
                        <p>👋 مرحباً بك!</p>
                        <p>أنا هنا لمساعدتك في {profile?.role === 'carrier' ? 'إدارة رحلاتك' : 'حجز رحلاتك'}.</p>
                        <p className="text-xs opacity-70">اسألني عن القوانين، الأسعار، أو طريقة الاستخدام.</p>
                    </div>
                )}
                {messages.map((m) => (
                    <div key={m.id} className={cn("flex gap-2", m.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                        ...
                        <div className={cn("p-3 rounded-lg text-sm max-w-[80%]", m.role === 'user' ? "bg-primary text-primary-foreground" : "bg-muted")}>
                            {m.content}
                        </div>
                    </div>
                ))}
                {isLoading && (
                    <div className="flex gap-2">
                        ...
                        <div className="p-3 rounded-lg bg-muted text-sm flex items-center">
                            <Loader2 className="h-4 w-4 animate-spin mr-2" /> جاري التفكير...
                        </div>
                    </div>
                )}
            </div>
