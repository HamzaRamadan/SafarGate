...
// // import { Flag, Ban, MessageSquare, Wallet, Star, Ticket, ChevronDown, Phone, Car, Image as ImageIcon, Building2 } from 'lucide-react';
...
// //             alt={carrier?.officeName || carrier?.firstName || "Carrier Avatar"}
// //           />
...
// //           <span className="text-muted-foreground font-normal">— {depositLabel}</span>
// //         </div>
...
// //           {(carrier?.officePhone || carrier?.phoneNumber) && (
// //             <div className="flex items-center gap-2">
// //               <Phone className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
// //               <span className="text-muted-foreground">رقم الهاتف:</span>
// //               <span className="font-medium ltr">{carrier?.officePhone || carrier?.phoneNumber}</span>
// //             </div>
// //           )}
...
// //                     <span className="font-medium">{carrier.vehicleType}</span>
// //                   </div>
// //                 )}
// //                 {carrier?.vehicleModel && (
// //                   <div className="flex gap-1">
// //                     <span className="text-muted-foreground">الموديل:</span>
// //                     <span className="font-medium">{carrier.vehicleModel}</span>
// //                   </div>
// //                 )}
// //                 {carrier?.vehicleYear && (
// //                   <div className="flex gap-1">
// //                     <span className="text-muted-foreground">السنة:</span>
// //                     <span className="font-medium">{carrier.vehicleYear}</span>
// //                   </div>
// //                 )}
// //                 {carrier?.plateNumber && (
// //                   <div className="flex gap-1">
// //                     <span className="text-muted-foreground">اللوحة:</span>
// //                     <span className="font-medium ltr">{carrier.plateNumber}</span>
// //                   </div>
// //                 )}
// //                 {carrier?.vehicleCapacity && (
// //                   <div className="flex gap-1">
// //                     <span className="text-muted-foreground">السعة:</span>
// //                     <span className="font-medium">{carrier.vehicleCapacity} مقعد</span>
// //                   </div>
// //                 )}
// //               </div>
// //             </div>
...
// //                 {carrier.vehicleImageUrls.map((url, i) => (
// //                   <img key={i} src={url} alt={`صورة المركبة ${i + 1}`} className="h-20 w-28 object-cover rounded-md shrink-0 border" />
// //                 ))}
// //               </div>
// //             </div>
// //           )}

// //           {!carrier?.officeName && !carrier?.officePhone && !carrier?.phoneNumber && !carrier?.vehicleType && !carrier?.vehicleImageUrls?.length && (
...
// //   // ✅ الـ 3 states للخطوات
// //   const [isEmailDialogOpen, setIsEmailDialogOpen] = useState(false);
// //   const [isBookingSheetOpen, setIsBookingSheetOpen] = useState(false);
// //   const [confirmedEmail, setConfirmedEmail] = useState('');

// //   // الخطوة 1: فتح popup الإيميل
// //   const handleBookClick = () => {
// //     if (onBookNow) setIsEmailDialogOpen(true);
// //   };

// //   // الخطوة 2: بعد تأكيد الإيميل، افتح البوكينج شيت
...
// //         {isBookable && (
// //           <>
// //             <CarrierDetailsAccordion
// //               trip={trip}
// //               depositLabel={`${t('deposit')}: ${trip.depositPercentage || 0}%`}
// //             />
...
// //                   {t('bookSeat')} ({trip.price} {trip.currency})
// //                 </>
// //               )}
// //             </Button>
// //           </>
...
//       {isOpen && (
//         <div className="p-3 space-y-3 bg-background">
//           {carrier?.officeName && (<div className="flex items-center gap-2"><Building2 className="h-3.5 w-3.5 text-muted-foreground shrink-0" /><span className="text-muted-foreground">المكتب:</span><span className="font-medium">{carrier.officeName}</span></div>)}
//           {(carrier?.officePhone || carrier?.phoneNumber) && (<div className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-muted-foreground shrink-0" /><span className="text-muted-foreground">رقم الهاتف:</span><span className="font-medium ltr">{carrier?.officePhone || carrier?.phoneNumber}</span></div>)}
//           {(carrier?.vehicleType || carrier?.vehicleModel || carrier?.vehicleYear || carrier?.plateNumber) && (
//             <div className="space-y-1.5">
...
//   // ✅ الخطوة 2: بعد كتابة الإيميل → بيبعت token والإيميل
...
//           confirmedEmail={confirmedEmail}
//           countryCode="JO"
//           tripDetails={{ origin: trip.origin || '', destination: trip.destination || '', carrierName: trip.carrierName || '' }}
//         />
//       )}
//     </>
//   );
...
export function ScheduledTripCard({
  ...
  // ✅ الخطوة 1: المستخدم يضغط "حجز مقعد" → يفتح ملخص الرحلة أولاً
  const handleBookClick = () => {
    if (onBookNow) setIsBookingSheetOpen(true);
  };

  // ✅ الخطوة 2: المستخدم يضغط "تأكيد الحجز" في الـ Sheet → يفتح البوبب بتاع الإيميل
  const handleSheetConfirm = () => {
    setIsBookingSheetOpen(false);
    setIsEmailDialogOpen(true);
  };

  // ✅ الخطوة 3: بعد كتابة الإيميل → بيبعت token والإيميل ويكمل الحجز
  const handleEmailConfirmed = async (email: string) => {
    if (!firestore) return;
    setIsSendingEmail(true);
    try {
