...
// import type { Trip, UserProfile, Booking, CarrierTier } from '@/lib/data';
...
//         // ✅ Guard هنا كمان قبل ما نفتح الـ sheet
//         if (!trip.carrierId) {
//             toast({
//                 variant: 'destructive',
//                 title: 'رحلة غير مكتملة البيانات',
//                 description: 'هذه الرحلة لا تحتوي على بيانات ناقل. الرجاء التواصل مع الدعم.',
//             });
//             return;
//         }
...
//   // ✅ الحجز الفعلي بيتم من صفحة confirm-booking بعد التحقق من الإيميل
...
//               {(searchOriginCity || searchDestinationCity) && (<Badge variant="secondary" className="mr-auto text-[10px] font-normal">{getCityName(searchOriginCity, locale) || t('all')} <span className="mx-1">→</span> {getCityName(searchDestinationCity, locale) || t('all')}</Badge>)}
//             </AccordionTrigger>
...
//                           </Label>
//                         <Select onValueChange={setSearchOriginCity} value={searchOriginCity} disabled={!searchOriginCountry}><SelectTrigger className="h-12 bg-card/50 border-muted"><SelectValue placeholder={t('fromCity')} /></SelectTrigger><SelectContent>{searchOriginCountry && (CITIES as any)[searchOriginCountry]?.cities.map((c: string) => (<SelectItem key={c} value={c}>{getCityName(c, locale)}</SelectItem>))}</SelectContent></Select>
//                       </div>
//                   </div>
//                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//                       <div className="space-y-2">
...
//                             <span className="opacity-0">.</span>
//                           </Label>
//                         <Select onValueChange={setSearchDestinationCity} value={searchDestinationCity} disabled={!searchDestinationCountry}><SelectTrigger className="h-12 bg-card/50 border-muted"><SelectValue placeholder={t('toCity')} /></SelectTrigger><SelectContent>{searchDestinationCountry && (CITIES as any)[searchDestinationCountry]?.cities.map((c: string) => (<SelectItem key={c} value={c}>{getCityName(c, locale)}</SelectItem>))}</SelectContent></Select>
//                       </div>
//                   </div>
...
// function setSelectedTripForBooking(trip: Trip) {
//   throw new Error('Function not implemented.');
// }
// function setIsBookingOpen(arg0: boolean) {
//   throw new Error('Function not implemented.');
// }
...
import { useState, useMemo, useEffect } from 'react';
import { useUser, useFirestore, useCollection } from '@/firebase';
import { collection, query, where, doc, documentId, limit, getDocs } from 'firebase/firestore';
import { useRouter } from '@/i18n/routing';
import { useTranslations, useLocale } from 'next-intl';
import { Search, ShipWheel, Filter, Car, Bus, LayoutGrid, Calendar as CalendarIcon, Send, ChevronsUpDown, Check, PlaneTakeoff, PlaneLanding } from 'lucide-react';
import { ScheduledTripCard } from '@/components/scheduled-trip-card';
...
import { AuthRedirectDialog } from '@/components/auth-redirect-dialog';
import { CITIES, getCityName } from '@/lib/constants';
import type { Trip, UserProfile, CarrierTier } from '@/lib/data';
import { useToast } from '@/hooks/use-toast';
import { ToastAction } from "@/components/ui/toast";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover';
...
export default function DashboardPage() {
  ...
  const activeBookingsQuery = useMemo(() => {
    if (!firestore || !user?.uid) return null;
    return query(collection(firestore, 'bookings'), where('userId', '==', user.uid), where('status', 'in', ['Pending-Payment', 'Pending-Carrier-Confirmation', 'Confirmed']));
  }, [firestore, user]);
  ...
  useEffect(() => {
    if (isLoadingBookings || isLoadingActiveRequests) return;
    if ((activeBookings && activeBookings.length > 0) || (activeRequests && activeRequests.length > 0)) {
      router.replace('/history');
    }
  }, [activeBookings, activeRequests, isLoadingBookings, isLoadingActiveRequests, router]);
  ...
  const tripsQuery = useMemo(() => {
    if (!firestore) return null;
    let dynamicConstraints: any[] = [where('status', '==', 'Planned')];

    if (rescueMode === 'time' && searchDate) {
      const startOfDay = new Date(searchDate); startOfDay.setHours(0, 0, 0, 0);
      const endOfRange = new Date(searchDate); endOfRange.setDate(endOfRange.getDate() + 3); endOfRange.setHours(23, 59, 59, 999);
      dynamicConstraints.push(where('departureDate', '>=', startOfDay.toISOString()));
      dynamicConstraints.push(where('departureDate', '<=', endOfRange.toISOString()));
      if (searchDestinationCity) dynamicConstraints.push(where('destination', '==', searchDestinationCity));
    } else if (rescueMode === 'location' && searchDestinationCity) {
      const targetCountryKey = Object.keys(CITIES).find(key => (CITIES as any)[key].cities.includes(searchDestinationCity));
      if (targetCountryKey) {
        dynamicConstraints.push(where('destination', 'in', (CITIES as any)[targetCountryKey].cities.slice(0, 10)));
        if (searchDate) {
          const start = new Date(searchDate); start.setHours(0,0,0,0);
          const end = new Date(searchDate); end.setHours(23,59,59,999);
          dynamicConstraints.push(where('departureDate', '>=', start.toISOString()));
    ...
    if (searchOriginCity) dynamicConstraints.push(where('origin', '==', searchOriginCity));
    if (searchSeats > 1) dynamicConstraints.push(where('availableSeats', '>=', searchSeats));
    if (searchMode === 'specific-carrier' && selectedCarrier) dynamicConstraints.push(where('carrierId', '==', selectedCarrier.id));
  ...
  useEffect(() => { if (!isUserLoading && !user) router.replace('/login'); }, [user, isUserLoading, router]);
  useEffect(() => { setRescueMode('none'); }, [searchOriginCity, searchDestinationCity, searchDate, searchVehicleType]);
  useEffect(() => { setSearchOriginCity(''); setSearchDestinationCountry(''); setSearchDestinationCity(''); }, [searchOriginCountry]);
  useEffect(() => { setSearchDestinationCity(''); }, [searchDestinationCountry]);

  const availableDestinations = useMemo(() => {
    if (!searchOriginCountry || !allTrips) return [];
    const reachableCountries = new Set<string>();
    allTrips.filter(trip => CITIES[searchOriginCountry as keyof typeof CITIES]?.cities.includes(trip.origin))
      .map(trip => trip.destination)
      .forEach(city => {
        const countryKey = Object.keys(CITIES).find(key => (CITIES as any)[key].cities.includes(city));
        if (countryKey && countryKey !== searchOriginCountry) reachableCountries.add(countryKey);
      });
    return Array.from(reachableCountries);
  }, [searchOriginCountry, allTrips]);
  ...
  const tripDisplayResult = useMemo((): TripDisplayResult => {
    const isSearching = !!(searchOriginCity || searchDestinationCity || searchDate || (searchMode === 'specific-carrier' && selectedCarrier));
    return { filtered: allTrips || [], showNoResultsMessage: isSearching && (!allTrips || allTrips.length === 0) };
  }, [allTrips, searchOriginCity, searchDestinationCity, searchDate, searchMode, selectedCarrier]);
  ...
  const carrierTierMap = useMemo(() => {
    const map = new Map<string, CarrierTier>();
    visibleCarriersData?.forEach((c) => { if (c.id && c.ratingStats?.tier) map.set(c.id, c.ratingStats.tier); });
    return map;
  }, [visibleCarriersData]);
  ...
  // ✅ بيتحقق من الحجوزات النشطة بس — الـ email flow بيتم في scheduled-trip-card
  const handleBookNowClick = async (trip: Trip) => {
    if (!user) { setIsAuthRedirectOpen(true); return; }
    if (!firestore) return;
    ...
    try {
      ...
      if (!snapshot.empty) {
        toast({ variant: "destructive", title: t('activeBookingExists'), description: t('activeBookingDesc'), duration: 5000 });
        return;
      }

      if (!trip.carrierId) {
        toast({ variant: 'destructive', title: 'رحلة غير مكتملة البيانات', description: 'هذه الرحلة لا تحتوي على بيانات ناقل.' });
        return;
      }

      // ✅ مفيش حاجة تاني هنا — scheduled-trip-card هيفتح EmailConfirmDialog بنفسه
    } catch (error) {
      console.error("Error checking active bookings:", error);
      toast({ variant: "destructive", title: tCommon('error'), description: tCommon('error') });
    } finally {
  ...
  const handleRequestAction = () => {
    ...
    if (!searchDate) missingFields.push(t('travelDate'));
    if (missingFields.length > 0) {
      toast({ variant: "destructive", title: t('requestIncomplete'), description: `${t('requestIncomplete')}: ${missingFields.join('، ')}.` });
      return;
    }
    setIsRequestOpen(true);
  };
  ...
  const handleRequestSent = () => {
    toast({ title: t('requestPublished'), description: t('requestPublishedDesc'), duration: 6000, action: <ToastAction altText={t('followRequest')} onClick={() => router.push('/history')}>{t('followRequest')}</ToastAction> });
    router.push('/history');
  };
      ...
      <div className="space-y-4">
            ...
            <AccordionContent className="p-4 pt-0 border-t">
              <div className="grid grid-cols-1 gap-4 pt-4">
                ...
                {searchMode === 'specific-carrier' && (
                  <Popover open={openCarrier} onOpenChange={setOpenCarrier}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" role="combobox" aria-expanded={openCarrier} className="w-full justify-between h-12 bg-card/50 border-muted">
                        {selectedCarrier ? `${selectedCarrier.firstName} ${selectedCarrier.lastName}` : t('searchCarrier')}
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-[300px] p-0" align="start">
                      <Command>
                        ...
                        <CommandGroup>
                          <CommandList>
                            {isLoadingCarriers ? <div className="p-2 text-sm text-muted-foreground text-center">{t('loading')}</div> : (
                              allCarriers?.map((carrier) => (
                                <CommandItem key={carrier.id} value={`${carrier.firstName} ${carrier.lastName}`} onSelect={() => { setSelectedCarrier(carrier); setOpenCarrier(false); }}>
                                  <Check className={cn("mr-2 h-4 w-4", selectedCarrier?.id === carrier.id ? "opacity-100" : "opacity-0")} />
                                  {carrier.firstName} {carrier.lastName}
                                </CommandItem>
                              ))
                            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Popover open={openOrigin} onOpenChange={setOpenOrigin}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" role="combobox" aria-expanded={openOrigin} className="w-full justify-between h-12 bg-card/50 border-muted">
                          {searchOriginCountry ? CITIES[searchOriginCountry as keyof typeof CITIES].name : t('selectOriginCountry')}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-[300px] p-0" align="start">
                        <Command>
                          ...
                          <CommandGroup>
                            <CommandList>
                              {Object.entries(CITIES).map(([key, { name }]) => (
                                <CommandItem key={key} value={name} onSelect={() => { setSearchOriginCountry(key); setSearchOriginCity(""); setOpenOrigin(false); }}>
                                  <Check className={cn("mr-2 h-4 w-4", searchOriginCountry === key ? "opacity-100" : "opacity-0")} />
                                  {name}
                                </CommandItem>
                              ))}
                            </CommandList>
                  ...
                  <div className="space-y-2">
                    <Label className="text-muted-foreground flex items-center gap-2"><span className="opacity-0">.</span></Label>
                    <Select onValueChange={setSearchOriginCity} value={searchOriginCity} disabled={!searchOriginCountry}><SelectTrigger className="h-12 bg-card/50 border-muted"><SelectValue placeholder={t('fromCity')} /></SelectTrigger><SelectContent>{searchOriginCountry && (CITIES as any)[searchOriginCountry]?.cities.map((c: string) => (<SelectItem key={c} value={c}>{getCityName(c, locale)}</SelectItem>))}</SelectContent></Select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Popover open={openDest} onOpenChange={setOpenDest}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" role="combobox" aria-expanded={openDest} disabled={!searchOriginCountry || availableDestinations.length === 0} className="w-full justify-between h-12 bg-card/50 border-muted disabled:opacity-50">
                          {searchDestinationCountry ? CITIES[searchDestinationCountry as keyof typeof CITIES]?.name : (!searchOriginCountry ? t('selectOriginFirst') : t('selectDestCountry'))}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-[300px] p-0" align="start">
                        <Command>
                          ...
                          <CommandGroup>
                            <CommandList>
                              {availableDestinations.map((key) => (
                                <CommandItem key={key} value={CITIES[key as keyof typeof CITIES].name} onSelect={() => { setSearchDestinationCountry(key); setSearchDestinationCity(""); setOpenDest(false); }}>
                                  <Check className={cn("mr-2 h-4 w-4", searchDestinationCountry === key ? "opacity-100" : "opacity-0")} />
                                  {CITIES[key as keyof typeof CITIES].name}
                                </CommandItem>
                              ))}
                            </CommandList>
        ...
        <div className="space-y-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center text-center py-10 px-4 border-2 border-dashed rounded-lg bg-secondary/5 text-muted-foreground animate-in zoom-in-95 duration-300">
              <div className="bg-background p-3 rounded-full mb-3 shadow-sm">{rescueMode === 'none' ? <ShipWheel className="h-6 w-6 text-primary/60" /> : <Search className="h-6 w-6 text-orange-500" />}</div>
              <h3 className="text-lg font-bold text-foreground mb-1">{rescueMode === 'none' ? t('noTripsExact') : t('noTripsExpanded')}</h3>
              <p className="text-xs max-w-xs mx-auto mb-6 text-muted-foreground">{rescueMode === 'none' ? t('tryFlexible') : t('createRequest')}</p>
              <div className="flex flex-col w-full max-w-sm gap-3">
                {rescueMode === 'none' && (
                  <>
                    <Button variant="outline" className="w-full justify-start gap-2 h-10 border-blue-200 hover:bg-blue-50 text-blue-700 transition-all" onClick={() => setRescueMode('location')}><LayoutGrid className="h-4 w-4" /><span>{t('searchWholeCountry')}</span></Button>
                    <Button variant="outline" className="w-full justify-start gap-2 h-10 border-orange-200 hover:bg-orange-50 text-orange-700 transition-all" onClick={() => setRescueMode('time')}><CalendarIcon className="h-4 w-4" /><span>{t('searchNext3Days')}</span></Button>
                  </>
                )}
                <Button className={cn("w-full justify-start gap-2 h-10 shadow-sm", rescueMode !== 'none' ? "bg-primary" : "mt-2 bg-slate-900 text-white hover:bg-slate-800")} onClick={handleRequestAction}><Send className="h-4 w-4" /><span>{rescueMode === 'none' ? t('createSpecialRequest') : t('sendOriginalRequest')}</span></Button>
                {rescueMode !== 'none' && <Button variant="ghost" size="sm" onClick={() => setRescueMode('none')} className="text-xs text-muted-foreground">{t('backToExact')}</Button>}
              </div>
            </div>
          ) : sortedTrips.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-16 border-2 border-dashed rounded-lg bg-secondary/10 text-muted-foreground animate-in zoom-in-95 duration-500">
              <div className="bg-background p-4 rounded-full mb-4 shadow-sm"><PlaneTakeoff className="h-8 w-8 text-primary/60" /></div>
              <h3 className="text-xl font-bold text-foreground mb-2">{t('marketWaiting')}</h3>
              <p className="text-sm max-w-sm mx-auto mb-6 leading-relaxed">{t('noScheduledTrips')}</p>
              <Button size="lg" className="gap-2 shadow-md hover:shadow-lg transition-all" onClick={handleRequestAction}><Send className="h-5 w-5" />{t('createFirstRequest')}</Button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in slide-in-from-bottom-4 duration-500">
              {sortedTrips.map(trip => (
