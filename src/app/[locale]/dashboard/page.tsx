...
//     try {
//         // ✅ Guard: تأكد إن carrierId موجود قبل الحجز
//         if (!trip.carrierId) {
//             toast({ variant: 'destructive', title: 'خطأ', description: 'بيانات الناقل غير مكتملة، يرجى المحاولة مرة أخرى.' });
//             throw new Error('carrierId is missing from trip data');
//         }

//         const bookingData: Omit<Booking, 'id' | 'createdAt' | 'updatedAt' | 'consentTimestamp'> = {
//             tripId: trip.id,
//             userId: user.uid,
//             carrierId: trip.carrierId,   // ✅ مضمون مش undefined بعد الـ guard
//             seats: searchSeats,
...
import { useState, useMemo, useEffect } from 'react';
import { useUser, useFirestore, useCollection } from '@/firebase';
import { collection, query, where, orderBy, doc, documentId, limit, getDocs, addDoc, serverTimestamp } from 'firebase/firestore';
import { useRouter } from '@/i18n/routing';
import { useTranslations, useLocale } from 'next-intl';
import { Search, ShipWheel, Filter, Car, Bus, LayoutGrid, Calendar as CalendarIcon, Send, ChevronsUpDown, Check, PlaneTakeoff, PlaneLanding } from 'lucide-react';
import { ScheduledTripCard } from '@/components/scheduled-trip-card';
...
export default function DashboardPage() {
  ...
  const tripsQuery = useMemo(() => {
    ...
    if (rescueMode === 'time' && searchDate) {
        ...
        if (targetCountryKey) {
            const citiesInCountry = (CITIES as any)[targetCountryKey].cities;
            dynamicConstraints.push(where('destination', 'in', citiesInCountry.slice(0, 10)));
            if (searchDate) {
                const start = new Date(searchDate); start.setHours(0,0,0,0);
                const end = new Date(searchDate); end.setHours(23,59,59,999);
                dynamicConstraints.push(where('departureDate', '>=', start.toISOString()));
  ...
  // ✅ يضغط "حجز المقعد" → يفتح EmailConfirmDialog مباشرة
  const handleBookNowClick = async (trip: Trip) => {
      if (!user) { setIsAuthRedirectOpen(true); return; }
      if (!firestore) return;
  ...
  // ✅ يكتب الإيميل → يبعت token → ينتظر
  const handleEmailConfirm = async (email: string) => {
    if (!firestore || !selectedTripForBooking) return;
    const trip = selectedTripForBooking;
    try {
      ...
      const confirmUrl = `${baseUrl}/${locale}/confirm-booking?token=${tokenDoc.id}`;
  ...
  const handleRequestSent = () => {
    toast({ 
        title: t('requestPublished'), description: t('requestPublishedDesc'), duration: 6000,
        action: <ToastAction altText={t('followRequest')} onClick={() => router.push('/history')}>{t('followRequest')}</ToastAction>
    });
    router.push('/history');
  };
  ...
  return (
    <AppLayout>
          ...
          {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-10 px-4 border-2 border-dashed rounded-lg bg-secondary/5 text-muted-foreground animate-in zoom-in-95 duration-300">
                ...
                <div className="flex flex-col w-full max-w-sm gap-3">
                    {rescueMode === 'none' && (
                        <>
                            <Button variant="outline" className="w-full justify-start gap-2 h-10 border-blue-200 hover:bg-blue-50 text-blue-700 transition-all" onClick={() => setRescueMode('location')}>
                                <LayoutGrid className="h-4 w-4" /><span>{t('searchWholeCountry')}</span>
                            </Button>
                            <Button variant="outline" className="w-full justify-start gap-2 h-10 border-orange-200 hover:bg-orange-50 text-orange-700 transition-all" onClick={() => setRescueMode('time')}>
                                <CalendarIcon className="h-4 w-4" /><span>{t('searchNext3Days')}</span>
                            </Button>
                        </>
                    )}
                    <Button className={cn("w-full justify-start gap-2 h-10 shadow-sm", rescueMode !== 'none' ? "bg-primary" : "mt-2 bg-slate-900 text-white hover:bg-slate-800")} onClick={handleRequestAction}>
                        <Send className="h-4 w-4" /><span>{rescueMode === 'none' ? t('createSpecialRequest') : t('sendOriginalRequest')}</span>
                    </Button>
                    {rescueMode !== 'none' && (
                        <Button variant="ghost" size="sm" onClick={() => setRescueMode('none')} className="text-xs text-muted-foreground">{t('backToExact')}</Button>
                    )}
                </div>
              </div>
          ) : sortedTrips.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-16 border-2 border-dashed rounded-lg bg-secondary/10 text-muted-foreground animate-in zoom-in-95 duration-500">
                  ...
                  <Button size="lg" className="gap-2 shadow-md hover:shadow-lg transition-all" onClick={handleRequestAction}>
                      <Send className="h-5 w-5" />{t('createFirstRequest')}
                  </Button>
              </div>
          ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {/* ✅ EmailConfirmDialog بدل BookingDialog مباشرة */}
