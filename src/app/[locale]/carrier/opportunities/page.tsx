...
//   const opportunities = useMemo(() => {
//     if (!rawOpportunities || !user) return [];
//     return rawOpportunities.filter(trip => 
//       trip.userId !== user.uid &&
//       (trip.passengers === undefined || trip.passengers <= (profile?.vehicleCapacity ?? Infinity)) &&
//       (!trip.preferredVehicle || trip.preferredVehicle === 'any' || trip.preferredVehicle === myCategory)
//     );
//   }, [rawOpportunities, user, profile, myCategory]);
...
export default function OpportunitiesPage() {
  ...
  const opportunitiesQuery = useMemo(() => {
    ...
    // لو الكابتن في رحلة نشطة، عرض طلبات من وجهته فقط
    if (profile.currentActiveTripId && activeTrip) {
       constraints.push(where('origin', '==', activeTrip.destination));
    }
