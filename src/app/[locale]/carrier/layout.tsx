...
export default function CarrierLayout({ children }: CarrierLayoutProps) {
  ...
  const navLinks = [
    ...
    { href: "/carrier/opportunities", label: t('market'), icon: Map, exact: false, count: 0, mobile: true },
    { href: "/carrier/bookings", label: t('requests'), icon: List, exact: false, count: pendingBookingsCount, mobile: true },
    { href: "/chats", label: t('messages'), icon: MessageSquare, exact: false, count: unreadChatsCount, mobile: true },
    ...
    { href: "/carrier/conditions", label: t('permanentConditions'), icon: ListChecks, exact: true, count: 0 },
  ];
      ...
      <div className="flex min-h-screen flex-col bg-background">
        <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container flex h-16 items-center justify-between px-4">
            ...
            {/* Desktop Nav Links */}
            <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
              {navLinks.filter(l => !l.mobile).map(item => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`transition-colors hover:text-primary ${pathname === item.href ? "text-primary" : "text-muted-foreground"}`}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className="relative h-9 w-9 rounded-full">
                      <Avatar className="h-9 w-9 border-2 border-primary/10">
                        <AvatarImage src={user?.photoURL || ""} alt={profile?.firstName || "Carrier"} />
                        <AvatarFallback className="bg-primary/5 text-primary">
                          {profile?.firstName?.[0] || "C"}
                        </AvatarFallback>
                      </Avatar>
                  ...
                  <DropdownMenuContent className="w-56" align="end" forceMount>
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem asChild className="cursor-pointer">
                      <Link href="/carrier/profile" className="flex items-center">
                        <User className="mr-2 h-4 w-4" />
                        <span>{t('carrierProfile')}</span>
                      </Link>
                    </DropdownMenuItem>
              ...
              <div className="hidden sm:flex items-center gap-2">
                <Button variant="ghost" size="icon" asChild className="relative ml-1">
                  <Link href="/chats">
                    <Bell className="h-5 w-5" />
                    {unreadChatsCount > 0 && (
                      <Badge variant="destructive" className="absolute -top-1 -right-1 h-4 w-4 flex items-center justify-center rounded-full p-0 text-[10px] animate-in zoom-in">
                        {unreadChatsCount > 9 ? "+9" : unreadChatsCount}
                      </Badge>
                    )}
                    <span className="sr-only">{t('messages')}</span>
        ...
        <CarrierBottomNav onAddTripClick={handleAddTripClick} navLinks={navLinks} />
      </div>
